...
class WpmfDisplayGallery
{
    ...
    public function getAttachmentLink(
        ...
        if (empty($remote_video)) {
            ...
            if (preg_match('/(youtube.com|youtu.be)\/(watch)?(\?v=)?(\S+)?/', $remote_video, $match)) {
        ...
        return '<a class="' . $class . '" data-lightbox="' . $lb . '" data-href="' . $url . '" title="' . esc_attr($title) . '" target="' . $target . '">' . $text . '</a>';
    }
    ...
    public function addEditorAssets()
    {
        ...
        wp_enqueue_script('jquery-masonry');
        global $pagenow;
        $deps = (isset($pagenow) && $pagenow === 'widgets.php') ? array('wp-blocks', 'wp-i18n', 'wp-element', 'wp-data', 'wp-block-editor', 'wordpresscanvas-imagesloaded') : array('wp-blocks', 'wp-i18n', 'wp-element', 'wp-data', 'wp-editor', 'wordpresscanvas-imagesloaded');
        wp_enqueue_script(
            'wpmf_gallery_blocks',
