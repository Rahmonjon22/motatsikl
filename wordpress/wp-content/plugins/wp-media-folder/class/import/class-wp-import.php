 ...
 */
class WPMF_Import extends WP_Importer
{
    ...
    {
        if (!empty($_POST['id'])) {
            $this->import_only_folder = (!empty($_POST['import_only_folder']) && (int) $_POST['import_only_folder'] === 1 && $this->allow_import_only_folder());
            $this->id = $_POST['id'];
            $file = $_POST['path'];
            $import_data = $this->parse($file);
            if (is_wp_error($import_data)) {
                $this->error_message .= '<p><strong>' . __('Sorry, there has been an error.', 'wpmf') . '</strong><br />';
            ...
            if ($this->version > $this->max_wxr_version) {
                ...
                $this->error_message .= sprintf(__('This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'wpmf'), esc_html($import_data['version']));
    ...
    function import_start($file)
    {
        if (!is_file($file)) {
            $this->error_message .= '<p><strong>' . __('Sorry, there has been an error.', 'wpmf') . '</strong><br />';
            $this->error_message .= __('The file does not exist, please try again.', 'wpmf') . '</p>';
            wp_send_json(array('status' => true, 'msg' => $this->error_message));
        }
    ...
    function import_end()
    {
        ...
        $this->error_message .= '<p>' . __('All done.', 'wpmf') . ' <a href="' . admin_url() . '">' . __('Have fun!', 'wpmf') . '</a>' . '</p>';
        $this->error_message .= '<p>' . __('Remember to update the passwords and roles of imported users.', 'wpmf') . '</p>';
    ...
    function process_posts()
    {
        ...
        foreach ($this->posts as $post) {
            ...
            if ($post_exists && get_post_type($post_exists) == $post['post_type']) {
                $this->error_message .= sprintf(__('%s &#8220;%s&#8221; already exists.', 'wpmf'), $post_type_object->labels->singular_name, esc_html($post['post_title']));
                $this->error_message .= '<br />';
                ...
                if (is_wp_error($post_id)) {
                    $this->error_message .= sprintf(__('Failed to import %s &#8220;%s&#8221;', 'wpmf'),
                ...
                foreach ($post['terms'] as $term) {
                    ...
                    if (!$term_id) {
                        ...
                        if (!is_wp_error($t)) {
                            ...
                            $this->error_message .= sprintf(__('Failed to import %s %s', 'wpmf'), esc_html($taxonomy), esc_html($term['name']));
                            if (defined('IMPORT_DEBUG') && IMPORT_DEBUG)
    ...
    function process_attachment($post, $url)
    {
        if ($this->import_only_folder)
            return new WP_Error('attachment_processing_error',
                __('Fetching attachments is not enabled', 'wpmf'));
        ...
        if ($info = wp_check_filetype($upload['file']))
            ...
            return new WP_Error('attachment_processing_error', __('Invalid file type', 'wpmf'));
    ...
    function fetch_remote_file($url, $post)
    {
        ...
        if (!$tmp_file_name) {
            return new WP_Error('import_no_file', __('Could not create temporary file.', 'wpmf'));
        }
        ...
        if (is_wp_error($remote_response)) {
            ...
            return new WP_Error(
                ...
                /* translators: 1: The WordPress error message. 2: The WordPress error code. */
                    __('Request failed due to an error: %1$s (%2$s)', 'wpmf'),
        ...
        if (200 !== $remote_response_code) {
            ...
            return new WP_Error(
                ...
                /* translators: 1: The HTTP error message. 2: The HTTP error code. */
                    __('Remote server returned the following unexpected result: %1$s (%2$s)', 'wpmf'),
        ...
        if (!$headers) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Remote server did not respond', 'wpmf'));
        }
        ...
        if (0 === $filesize) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Zero size file downloaded', 'wpmf'));
        }

        if (!isset($headers['content-encoding']) && isset($headers['content-length']) && $filesize !== (int)$headers['content-length']) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('Downloaded file has incorrect size', 'wpmf'));
        }

        $max_size = (int)$this->max_attachment_size();
        if (!empty($max_size) && $filesize > $max_size) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', sprintf(__('Remote file is too large, limit is %s', 'wpmf'), size_format($max_size)));
        }
        ...
        if ((!$type || !$ext) && !current_user_can('unfiltered_upload')) {
            return new WP_Error('import_file_error', __('Sorry, this file type is not permitted for security reasons.', 'wpmf'));
        }
        ...
        if (!$move_new_file) {
            @unlink($tmp_file_name);
            return new WP_Error('import_file_error', __('The uploaded file could not be moved', 'wpmf'));
        }
