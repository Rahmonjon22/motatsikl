 ...
 */
let wpmfFoldersModule, wpmfAddCloudQueue, cloud_sync_loader_icon;
(function ($) {
    wpmfFoldersModule = {
                ...
                $.extend(wp.Uploader.prototype, {
                    init: function () {
                        ...
                        this.uploader.bind('FilesAdded', function (up, files) {
                            ...
                            let percent = Math.round(100*wpmfFoldersModule.uploaded_files_length/wpmfFoldersModule.added_files_length) + '%';
                            wpmfSnackbarModule.show({
                                id: 'uploading_media',
        ...
        getBackboneOfMedia: function() {
            let t, a, n = wpmfFoldersModule.getFrame().parents(".media-modal");
         ...
         */
        getFolderRendering: function (type, name, term_id, parent_id) {
            let buttons = '';
            let class_names = '';
            let main_icon = '';
            let action = '';
        ...
        renderFolders: function (term_id) {
            if (parseInt(wpmfFoldersModule.enable_folders) === 0) { // return if disable show folders option
                return;
            }
            ...
            $(folders_ordered).each(function () {
                ...
                if (parseInt(wpmf.vars.hide_tree) === 1) {
                    if (this.drive_type === '' || (this.drive_type !== '' && parseInt(this.parent_id) !== 0)) {
                        let folder = wpmfFoldersModule.getFolderRendering('folder', this.label, this.id, this.parent_id);
        ...
        openContextMenuFile: function () {
            ...
            $('.attachments-browser .attachment:not(.wpmf-attachment)').off('contextmenu').on('contextmenu', function (e) {
                ...
                $('.material_downloadfile').attr('href', wp.media.attachment(wpmfFoldersModule.editFileId).get('url'));
                ...
                if (wp.media.attachment(wpmfFoldersModule.editFileId).get('mime') === 'image/jpg'
                    || wp.media.attachment(wpmfFoldersModule.editFileId).get('mime') === 'image/jpeg'
                    || wp.media.attachment(wpmfFoldersModule.editFileId).get('mime') === 'image/png') {
                ...
                let context_height = $('.wpmf-contextmenu.wpmf-contextmenu-file').height();
        ...
        renderPermissionHtml: function(role_user_list, cl = 'folder_permission_items') {
            return `<div class="folder_permission_items_wrap"><div class="${cl}" data-role="{{wpmf_role}}">
                ...
                </div><hr class="wpmf_hr"/></div>`;
        },

        renderFolderPermissionsWrap: function () {
            ...
            let role_list = `<div class="width_100">
                        <div class="ju-settings-option folder_permission_item">
                            <div class="wpmf_row_full">
            ...
            let user_list = `<div class="width_100">
                        <div class="ju-settings-option folder_permission_item">
                            <div class="wpmf_row_full">
            ...
            let role_permission_html = wpmfFoldersModule.renderPermissionHtml(role_list, 'folder_permission_items folder_role_permission_items');
            let user_permission_html = wpmfFoldersModule.renderPermissionHtml(user_list, 'folder_permission_items folder_user_permission_items');
            let html = `
            <div id="folder_permissions" class="wpmf_view folder_permissions_wrap white-popup mfp-hide">
                <div class="folder_permissions_box wpmf_scrollbar_style">
                    ...
                    <p class="folder_permission_items_wrap"><button class="wpmf_add_role ju-button">
                            ${wpmf.l18n.add_role}
                    ...
                    <p class="folder_permission_items_wrap"><button class="wpmf_add_user ju-button">
                            ${wpmf.l18n.add_user}
                            <span class="material-icons-outlined">add_circle_outline</span>
                        </button></p>
                        </div>
                                    ...
                                    if (response.status) {
                                        ...
                                        if (response.role_permissions.length) {
                                            $.each(response.role_permissions, function (i, permission) {
                                                let selected = permission[0];
                                                if (selected == null) {
                                                    selected = 0;
                                                }
                                                let html = role_permission_html.replace('{{wpmf_role}}', selected);
                                        ...
                                        if (response.user_permissions.length) {
                                            $.each(response.user_permissions, function (i, permission) {
                                                ...
                                                let html = user_permission_html.replace('{{wpmf_role}}', selected);
...
<div class="ju-settings-option folder_permission_item folder_permission_inherit_folder_item">
                            ...
                            <div class="ju-switch-button">
                                <label class="switch">
                                    ...
                                    <span class="slider round"></span>
                                        ...
                                        $('.wpmf_user_permissions, .wpmf_role_permissions').select2();
        ...
        removeRolePermissions: function() {
        ...
        renderContextMenu: function () {
            ...
            if (wpmf.vars.enable_download_media) {
                download_media = `<li><a download href="" class="material_downloadfile items_menu">${wpmf.l18n.download}<span class="material-icons-outlined wpmf_icon"> get_app </span></a></li>`
            }
        ...
        initializeDragAndDropAttachments: function () {
            ...
            let placeholder = '';
            let accept = '.attachment:not(.wpmf-back):not(.wpmf-new), #the-list tr';
        ...
        sortableFolder: function ($frame, append_element) {
            let placeholder = 'mdc-list-item attachment wpmf-attachment material_design wpmf-folder mdc-ripple-upgraded wpmf-transparent';
            if (wpmfFoldersModule.page_type !== 'upload-list') {
                $('.attachments').sortable();
        ...
        droppedAttachment: function (to_folder_id) {
            if ($(wpmfFoldersModule.dragging_elements).hasClass('wpmf-folder')) {
                ...
                $(wpmfFoldersModule.dragging_elements).each(function () {
                    if (wpmfFoldersModule.page_type === 'upload-list') {
                        let row_ids = $(this).attr('id');
                        let row_id = row_ids.split('-');
                        let file_id = parseInt(row_id[1]);
         ...
         * @return jqXHR
         */
        moveFile: function (files_ids, folder_to_id, folder_from_id, undo = false) {
        ...
        doMoveFile: function(files_ids, folder_to_id, folder_from_id, undo) {
            $.ajax({
                type: "POST",
                url: wpmf.vars.ajaxurl,
                data: {
        ...
        initAttachmentLabelS3: function () {
            ...
            wpmfFoldersModule.getFrame().find('.attachments-browser ul.attachments .attachment .thumbnail').each(function (i, v) {
                ...
                let id = $wrap.data('id');
                let src_img = wp.media.attachment(id).get('url');
                let aws3_infos = wp.media.attachment(id).get('aws3_infos');
                if (typeof src_img !== "undefined") {
                    if (typeof aws3_infos !== "undefined") {
                        if (!$wrap.find('.wpmf_aws_text').length) {
                            ...
                            let aws3_tooltip = `<div class="wpmf_aws_text_info" id="tippy_${id}">${aws_text_info}</div>`;
            ...
            tippy('.wpmf_aws_text', {
                ...
                allowHTML: true,
                onShow(instance) {
                    let id = $(instance.reference).data('wpmftippy_id');
