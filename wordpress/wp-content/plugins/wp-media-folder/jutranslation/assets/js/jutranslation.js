...
jQuery(document).ready(function($){
    ...
    $.each(julanguages, function(){
        ...
        $.ajax({
            ...
            // Filter the old revision on same version
                var languageVersions = {};
                $.each(data.data, function () {
                    if (languageVersions.hasOwnProperty(this.language + '-' + this.extension_version)) {
                        // Compare revision
                        if (parseInt(this.revision) > parseInt(languageVersions[this.language + '-' + this.extension_version]['revision'])) {
                            // Replace by new version
                            languageVersions[this.language + '-' + this.extension_version] = this;
                        }
                    } else {
                ...
                $('#jutranslations-languages tbody tr[data-slug="'+ julanguage.extension + '"]').each(function(){
                    ...
                    $.each(Object.values(languageVersions),function(){
