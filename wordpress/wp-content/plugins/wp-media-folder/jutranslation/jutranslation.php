...
class Jutranslation
{
    ...
    public static function init($main_plugin_file, $extension_slug, $extension_name, $text_domain, $language_file)
    {
        ...
        if (is_admin()) {
            if (!function_exists('wp_get_current_user') && !is_multisite()) {
    ...
    {
        /**
         * Filter check capability of current user to translate
         *
         ...
         */
        $wpmf_capability = apply_filters('wpmf_user_can', current_user_can('manage_options'), 'translate_plugin');
        if ($wpmf_capability) {
            //Initialize needed ajax mehtods
            add_action(
                'wp_ajax_jutranslation_' . self::$extension_slug,
    ...
    protected static function saveStrings()
    {
        ...
        if (!preg_match('/^[a-z]{2,3}(?:-[a-zA-Z]{4})?(?:-[A-Z]{2,3})?(-formal)?$/', $language)) {
    ...
    protected static function showViewForm()
    {
        ...
        //Check if a translation exists for each entry
        foreach ($mo->entries as $entry) {
            $singular = str_replace('&quot;', '', $entry->singular);
            $translations = str_replace('&quot;', '', $entry->translations[0]);
            $strings[$singular] = $translations;
        }
        ...
        echo '<textarea style="display: none" name="strings">' . htmlentities($strings, ENT_COMPAT, 'UTF-8') . '</textarea>';
        echo '</form>';
        //Add waiting image
